...
`nodemailer` 패키지와 `EMAIL_*` 변수는 남아 있다 — 개발자 모드 회원가입의 **인증 메일**을
그것으로 보낸다(14.5). 용도만 로그인에서 가입 인증으로 옮겨 갔다.
# 화면 설계서
...
| ID | 화면 | 경로 | 렌더링 | 진입점 |
|---|---|---|---|---|
| S-00 | 대시보드 (nav + 상세 + 시장 타일) | `/dashboard` · `/dashboard/[code]` | 서버 (레이아웃 = nav·타일, 페이지 = 상세) | **로그인 직후** · 탭바 · 제호 액션 — 셋 다 로그인했을 때만 |
| S-01 | 시장 현황 | `/` | 서버, `force-dynamic` | 제호 · 탭바 시장 · 404 복귀 |
| S-02 | 종목 상세 — 에디토리얼 | `/stocks/[symbol]` | 서버 | 검색 · 등락 목록 · nav · 브리핑 |
| S-03 | 종목 상세 — 터미널 콘솔 | `/stocks/[symbol]?view=console` | S-02 와 동일 렌더 | 뷰 토글 · 딥링크 |
| S-05 | 로그인 · 회원가입 · 이메일 인증 | `/login` · `/signup` · `/verify` | 서버 | 계정 메뉴 · 대시보드 게이트. 가입·인증은 **개발자 모드에서만** |
...
S-00 · S-01 · S-02 · E-03 이 같은 골격을 공유한다.
...
│                                                        │
└────────────────────────────────────────────────────────┘
   MobileTabBar  (대시보드) · 시장 · 검색 · 탐색   <768 에서만, fixed bottom
```

| 요소 | 사양 | 구현 |
|---|---|---|
| 컨테이너 | **`max-w-shell`**(= `--shell: 1440px`), 좌우 `px-4` → `md:px-8`, 상단 `pt-[26px]` | 각 `page.tsx` · 토큰은 [globals.css](../src/app/globals.css) |
| 하단 여백 | 기본 `pb-[30px]`. 모바일 고정 요소가 있는 화면은 `pb-24`(상세) / `pb-28`(홈·404) / `pb-40`(대시보드) | 고정 바에 콘텐츠가 가리지 않게 |
...
| 탭바 | 높이 `--tabbar-h: 62px`, `bottom-0 z-30`, `md:hidden`. 탭 44×44. 로그인하면 4개(대시보드 · 시장 · 검색 · 탐색), 아니면 3개 | [MobileTabBar.tsx](../src/shared/components/layout/MobileTabBar.tsx) |
...
`/` 의 탭 라벨은 **홈 → 시장**이다. 대시보드와 나란히 서면 둘 다 "첫 화면" 으로 읽혀
무엇이 다른지 알 수 없다. 제호(`Wordmark`)는 여전히 `/` 로 간다.
...
| 파일 | 역할 |
|---|---|
| [layout.tsx](../src/app/dashboard/layout.tsx) | 로그인 게이트 · 제호 · nav · 시장 타일. **자식 간 이동에서 다시 렌더되지 않는다** |
| [_data/watchlist.ts](../src/app/_data/watchlist.ts) | `loadWatchlist` — React `cache` 로 요청당 1회 |
| [page.tsx](../src/app/dashboard/page.tsx) | `/dashboard` → nav **맨 위 종목**의 상세 |
| [[code]/page.tsx](../src/app/dashboard/[code]/page.tsx) | `/dashboard/005930` → 그 종목 상세 |
| [loading.tsx](../src/app/dashboard/loading.tsx) · [error.tsx](../src/app/dashboard/error.tsx) | **상세 칸만**의 골격·오류 경계 (레이아웃 밖이 아니라 안이다) |
| [DashboardBoard.tsx](../src/app/dashboard/_components/DashboardBoard.tsx) | 상태 소유자 — 보기·그룹·정렬·선택·일괄 AI |
| [DashboardDetail.tsx](../src/app/dashboard/_components/DashboardDetail.tsx) | 상세 조립 + AI 드로어 |
| [MarketTiles.tsx](../src/app/dashboard/_components/MarketTiles.tsx) | 지수 · 등락 상위 · 오늘의 일정 · AI 판단 현황 |
...
| [layout.tsx](../src/app/dashboard/layout.tsx) `currentUser()` | 진짜 판단. 신원이 없으면 `redirect("/login")` | — |
...
┌─ Masthead ─────────────────────────────── 검색 · 테마 · 계정 ─┐
├───────────────────────────────────────────────────────────────┤
│ WatchlistHeader   관심 종목 Watchlist   [순서 편집][＋추가]   │
│ GroupTabs 전체│코어│…            [분할│표]  SortControl ▾     │
├──────────────────────┬────────────────────────────────────────┤
│ nav (WatchRowCompact)│  StockDetailBody variant="flow"        │
│  ▸ 삼성전자   +1.42% │   헤드라인 · [전체 상세][담기][AI]      │
│  ● SK하이닉스 +2.86% │   차트                                  │
│  ▸ NAVER      +0.10% │   투자 지표 │ 밸류에이션  (가로 2칸)    │
│                      │   뉴스 / 리포트 / 재무 탭               │
├──────────────────────┴────────────────────────────────────────┤
│ MarketTiles   지수 4카드                                       │
│               상승률 상위 │ 하락률 상위                        │
...
**시장 타일은 두 열 아래, 전체 폭이다.** 오른쪽 칸 안(상세 아래)에 넣으면 1012px 로
좁아지는데 지수 4카드·등락 2열은 넓을수록 읽기 쉽고, 무엇보다 그 칸이 `hidden md:grid`
라서 **모바일에서 통째로 사라진다.** 아래에 두면 폭도 얻고 모든 폭에서 보인다.

**표 보기**로 넘기면 nav 와 상세가 빠지고 전체 폭 8열 `TableHeader` + `WatchRow` 가
선다. 컬럼 폭은 `WATCH_GRID` 상수 하나를 헤더와 데이터 행이 공유한다(어긋남 방지).
...
| 전역 | 관심 추가 저장 경로 이원화 | 검색 팔레트의 ⇥ 는 `store/watchlist.ts`(zustand persist → localStorage)에 쌓고, 화면의 담기(`WatchToggle`)는 `/api/watchlist` 로 간다. 두 경로를 하나로 합쳐야 한다 |
